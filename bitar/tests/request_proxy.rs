...
async fn request_proxy_on() -> Result<(),Box<dyn std::error::Error>>{
    ...
    let client_build = reqwest::Client::builder().add_root_certificate(cert).proxy(reqwest::Proxy::http("socks5://127.0.0.1:1080").expect("failed to socks5 proxy socks5"));
    let client= client_build.build().expect("failed to build client");
    println!("sleep 15 start");
    tokio::time::sleep(tokio::time::Duration::from_secs(15)).await;
    println!("sleep 15 end");
    let response = client.get(url).send().await.expect("request failed");
    let mut content_length = 0 as usize;
    response.headers().iter().for_each(|(k,v)|{
        if reqwest::header::CONTENT_LENGTH == k {

            let value = v.to_str().expect("failed to convert content length to str");
            content_length = value.parse::<usize>().expect("failed to convert content length to usize");
        }
    });
    let bytes = response.bytes().await.expect("response bytes err");
    println!("sleep 50 start");
    tokio::time::sleep(tokio::time::Duration::from_secs(50)).await;
    println!("sleep 50 end");
